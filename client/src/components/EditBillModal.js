...
import './AddBillModal.css'; // Reusing the same styles

const EditBillModal = ({ isOpen, onClose, onSave, bill, patients }) => {
  ...
  useEffect(() => {
    if (bill) {
      const servicesArray = Array.isArray(bill.services) 
        ? bill.services.map(service => ({ name: service, cost: '' }))
        : [{ name: bill.services || '', cost: '' }];
  ...
  const handleInputChange = (e) => {
    ...
    // Auto-fill patient name when patient ID is selected
    if (name === 'patientId') {
  ...
  const handleServiceChange = (index, field, value) => {
    ...
    // Calculate total amount if cost is changed
    if (field === 'cost') {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      const updatedBillData = {
        ...
        services: formData.services.map(s => s.name).filter(name => name.trim()),
        updatedAt: new Date().toISOString()
      };
      
      onSave(updatedBillData);
      handleClose();
    }
  };
